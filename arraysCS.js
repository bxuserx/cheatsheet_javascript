/*
    Workbook 3/N:  Numerics and Strings in arrays.
...
var num_ind = [];                               // Finding all the indicies in the matrix that === 56
var idx = numbers.indexOf(56);
while (idx != -1) {
  num_ind.push(idx);
  idx = numbers.indexOf(56, idx + 1);           // indexOf(searched element, fromIndex). Search starting at next element.
}
...
let str_hasApe = strArr.some(x => x === `ape`); // True/False if array contains one element = ape
let str_allApe = strArr.every(x => x === `ape`);// True/False if every array element = ape
